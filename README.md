...
This artifact requires significant amounts of memory and compute cores to run. The standard experiments can be run on 4 cores and at least 16 GB of memory. The full experiments require at least 100 GB of memory and 24 cores to run in a timely manner.
...
In order to smoke test this artifact, follow the steps in [Getting ToVer](#getting-tover), then execute the standard experiments as explained in [Running experiments](#running-experiments). This should take about an hour on 4 cores with 16GB of memory. 

This should generate statistics in the folder `stats/` which can either be inspected using [JupyterLab](http://127.0.0.1:8080/lab/), or using the command line opened by starting the docker container. 

The plots can then be generated by running following the steps as described in [Analyzing the Results](#analyzing-the-results).
...
For a POMDP:
```bash
python -m verimon.run --file tests/premise/airportA-3.nm --loader pomdp --constants "DMAX=3,PMAX=3" --spec "Pmax=? [F<=4 crash]" --good_label crash --threshold 0.3 --horizon 10 --relative_error 0.01 --use_risk --fp_slack 0.2 --fn_slack 0.05 --use_random_eq --walks_per_state 100 --walk_len 11 --base_dir results/airport_experiment
```

For SnLs:
```bash
python -m verimon.run --file tests/mc_u_nxn.pm --loader snakes_ladders --n 100 --ladders "1:38,4:14,9:31,28:64,40:42,36:44,51:67,71:91,80:100" --snakes "98:76,95:75,93:73,87:24,64:60,62:19,55:53,49:11,47:26,16:6" --spec "Pmax=? [F<5 good]" --good_label good --threshold 0.3 --horizon 15 --relative_error 0.01 --use_risk --fp_slack 0.2 --fn_slack 0.05 --use_random_eq --walks_per_state 100 --walk_len 11 --base_dir results/snakes_ladders_experiment
```
...
- `--base_dir results/airport_experiment` defines where to save the statisics, models and dot file of the model.

## Running experiments
Our experiments are defined in yaml files and consumed by the python file `verimon/experiments.py`. 

The file `tests/standard-exp/verify.yml` contains the standard version of the experiments for the section "Efficiency of Monitor Verification", and the folder `tests/verify-exp/` contains the extended version of the experiments found in the verification secion of the paper.

The file `tests/standard-exp/learn.yml` contains the standard version of the experiments for the section "Efficiency of Monitor Learning", and the folder `tests/learn-exp/` contains the extended version of the experiments found in the learning section.


For obtaining the standard results run the following command.
```bash
python -m verimon.experiment tests/standard-exp/* -c -t 9300
```
This exectues all experiments found the the folder `tests/standard-exp/` concurrently, each with a timeout of 10 minutes (600 seconds).
...
Our results are stored in the folders `results/exp-2025-04-15_15-00-29-comp-base-prem_sam-premise-snl-snl_sam` and `results/exp-2025-04-24_12-13-12-verify`. These are the exact results used in the paper.

These results can be analyzed by opening the `papep.ipynb` [notebook](http://127.0.0.1:8080/lab/tree/notebooks/paper.ipynb) and executing all cells. All figures are show in the notebook labeled by which figure they represent. The figures are also exported as `.pgf` files at `VerifiableMonitorsTex/inprogress/images/plots`, which are directly included in the paper.
