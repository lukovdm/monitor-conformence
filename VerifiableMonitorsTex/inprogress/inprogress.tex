...
\begin{itemize}
    ...
    \item Translation to CTP problem (refer to Complexity section)
    \item MDPs and translation to consisitent policy synthesis for MDPs
    \item Solving consistent policy synthesis for MDPs using PAYNT
\end{itemize}
...
The monitor verification problem consists of two sub problems. The no false alarms problem, and the no missed alarms problem. Both of these sub problems entail finding the trace with either the minimum or maximum risk. We will discuss the missed alarm problem in the following sections. How to adapt to no false alarms can be found in \cref{ssec:nfa}.

The no false alarms problem entails ensuring that all traces from the MC, which are in the monitor, have at least a risk $\lambda^{fa}$, the \emph{false alarm threshold}. The no missed alarms problem entails instead, that traces not in the monitor have at most a risk $\lambda^{ma}$, the \emph{missed alarm threshold}.

\begin{mdframed}
    \textbf{Monitor verification problem}\\
    Given a HMM $\mc$ with a state risk function $r: \Sts \to \RR_{\leq 0}$, a DAG monitor DFA $\dfa$ with as alphabet the observations of $\mc$, a pair of risk thresholds $\lambda^{fa}, \lambda^{ma}\in[0,\inf)$, and a horizon $h\in\NN^+$. Decide both $R_r^{\min}(\dfa) > \lambda^{fa}$ and $R_r^{\max}(\overline{\dfa}) < \lambda^{ma}$, where the \emph{weighted extreme risk function} $R_r^{\min/\max}\colon DFA \to \RR_{\leq 0}$ is defined as
    \[
        R_r^{\min/\max}(\dfa) := \underset{\tau\in\lang{\mc}\cap\lang{\dfa}}{\min/\max} \sum_{\pi\in\Pi^h_\mc}\Prob(\pi \ |\ \tau) \cdot r(\pi_{\downarrow})
    \]
\end{mdframed}
...
\todo[inline]{Add assumption of no transitions to the initial state}

We transform the HMM into a POMDP in which you choose which color states to transition into. The observations in the POMDP will not be the colors of the states, but instead the amount of steps from the initial state. Finally, we will add end actions to all states, if the state we end in is in $G$, we transition to a goal state, otherwise we transition to a stop state. We detail this transformation in two steps.
\todo[inline]{Can also unfold here on the horizon and use that for the observations.}

\paragraph{Step 1}
Transition function of the POMDP becomes:
\begin{align*}
    P'(s, c, q) :=
    \begin{cases}
    ...
    \end{cases}
\end{align*}
The observation function of the POMDP becomes the amount of steps from the initial state to the current state.
\begin{remark}
    Note that any states at the horizon automatically get a return transition to the initial state with probability 1.
\end{remark}

\paragraph{Step 2}
We add two states to the POMDP, a goal state and a stop state. Both are sink states.
...
\subsection{Performing policy synthesis using memoryless policies}
